...
export const AppHttpRequests = () => {
  ...
  const changeTaskStatusHandler = (e: ChangeEvent<HTMLInputElement>, task: DomainTask, todolistId: string) => {
    ...
    tasksApi.changeTask({ task, todolistId, model }).then((res) => {
      console.log(res.data)
      setTasks({
        ...tasks,
        [todolistId]: tasks[todolistId].map((task) =>
          task.id === res.data.data.item.id ? { ...task, status: model.status } : task,
        ),
      })
    })
  }
  const changeTaskTitleHandler = (title: string, task: DomainTask, todolistId: string) => {
    ...
    tasksApi.changeTask({ task, todolistId, model }).then((res) => {
      ...
      setTasks({
        ...tasks,
        [todolistId]: tasks[todolistId].map((task) =>
          task.id === res.data.data.item.id ? { ...task, title: model.title } : task,
        ),
      })
    })
  }
      ...
      {todolists.map((tl) => {
        return (
          <div key={tl.id} style={todolist}>
            <div>
              <EditableSpan title={tl.title} callback={(title: string) => updateTodolistHandler(tl.id, title)} />
              ...
              tasks[tl.id].map((task: DomainTask) => {
                return (
                  <div key={task.id}>
                    ...
                    <EditableSpan title={task.title} callback={(title) => changeTaskTitleHandler(title, task, tl.id)} />
